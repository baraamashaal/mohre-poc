...
const QuoteIcon: FC<{ className?: string }> = ({ className }) => {
  return (
    <div className={className}>
      <svg className="quote-icon" width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
...
export const Blockquote: FC<BlockquoteProps> = ({ children, cite, author, authorTitle, style = 'soft' }) => {
  try {
  ...
  return (
    <blockquote
      cite={cite}
      className={`relative p-6 text-base font-medium text-gray-800 ${
        style === 'solid'
    ...
    >
      {style === 'soft' && (
        <QuoteIcon className="absolute left-0 top-0 h-12 w-12 text-gray-800 rtl:left-auto rtl:right-0 rtl:scale-x-[-1]" />
      )}
      <p className="text-gray-800">{children}</p>
      {(author ?? authorTitle) && (
        <footer className="mt-10">
          {author && <div className="mb-1 text-sm font-medium text-primary-700">{author}</div>}
          {authorTitle && <div className="text-sm font-normal text-primary-600">{authorTitle}</div>}
        </footer>
      )}
    </blockquote>
  )
