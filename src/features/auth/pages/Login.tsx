  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-aegov-bg px-4 py-8">
      ...
      <div className="w-full sm:w-[26rem] mx-auto p-6 text-center space-y-10 border border-primary-400 rounded md:rounded-lg xl:rounded-xl min-h-[32rem] xl:min-h-[35rem] flex flex-wrap justify-between flex-col content-between [&>*]:w-full" id="accordion-collapse" data-accordion="collapse">
        ...
        <div className={`space-y-3 login-content ${showAlternativeLogin ? 'hidden' : ''}`} id="aegov-accordion-body-1" aria-labelledby="aegov-accordion-head-1">
          <button className="aegov-btn btn-sm lg:btn-base btn-secondary" type="button" onClick={handleUAEPassLogin}>
            <svg xmlns="http://www.w3.org/2000/svg" width="26" height="25" viewBox="0 0 26 25" fill="none">
              ...
              <path fillRule="evenodd" clipRule="evenodd" d="M4.23685 5.159C4.60657 5.159 4.90754 4.86575 4.90754 4.50492C4.90754 4.14432 4.60657 3.85083 4.23685 3.85083C3.86713 3.85083 3.56616 4.14432 3.56616 4.50492C3.56616 4.86575 3.86713 5.159 4.23685 5.159Z" fill="white" />
          ...
          <p className="text-sm px-6 text-aeblack-800">A single trusted digital identity for all citizens, residents and visitors.</p>
        </div>

        {/* Alternative Login Section */}
        <div className={`login-content ${!showAlternativeLogin ? 'hidden' : ''}`} id="aegov-accordion-body-2" aria-labelledby="aegov-accordion-head-2">
          <div role="alert" id="alert-4" className="aegov-alert alert-warning mb-6">
            <div className="alert-content">
              <p className="text-sm">This login method will be discontinued soon. You must switch to the use of UAE PASS.</p>
            </div>
          ...
          <form action="" className="text-start" onSubmit={(e) => {
            e.preventDefault()
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                <a href="#" onClick={(e) => { e.preventDefault(); }} className="no-underline">Forgot Password?</a>
              </div>

              <div>
                <button type="submit" className="aegov-btn btn-block btn-sm lg:btn-base mt-6">Login</button>
              </div>
            </div>
          </form>
        </div>
        ...
        {/* Footer Links */}
        <div>
          <div className={`aria-expanded:hidden pointer-events-none ${showAlternativeLogin ? 'hidden' : ''}`} data-accordion-target="#aegov-accordion-body-2" aria-expanded="false" aria-controls="aegov-accordion-body-2">
        ...
        <div className={`aria-expanded:hidden ${showAlternativeLogin ? '' : 'hidden'}`} data-accordion-target="#aegov-accordion-body-1" aria-expanded="true" aria-controls="aegov-accordion-body-1">
